...
MobilityHelper::MobilityHelper(Rover* rover, ros::NodeHandle* nh, Stoke* stoke)
{


    rover_ = rover;
    uniform_ = new Uniform(rover_->getName());
    pubs = new Publishers;
    subs = new Subscribers;
    timers = new Timers;
    state = TRANSFORM;
        ...
        switch(state)
        {
            case TRANSFORM:
            {
                ...
                //Otherwise, assign a new goal
                    ...
                    goalLocation = stoke_->getPosition();
                    // set the center for stoke about 1.5m from the center
                    
                }
                else if(searchStyle == 1){
                    goalLocation.theta = M_PI + atan2(currentLocation.y, currentLocation.x);
                    goalLocation.x = .5 * cos(goalLocation.theta);
                    goalLocation.y = .5* sin(goalLocation.theta);
                    searchStyle = 0;
                }
                else{
                    goalLocation.theta = M_PI + atan2(currentLocation.y, currentLocation.x);
...
void MobilityHelper::targetHandler(const shared_messages::TagsImage::ConstPtr& message)
{
    ...
    {       
        ...
        //check if target has not yet been collected
            ...
            state = TRANSFORM;
            for (int i = 1; i < sizeof(message->tags.data); i++)
            {
              geometry_msgs::Pose2D currentLocation = rover_->getPosition();
              TagObject tag;
